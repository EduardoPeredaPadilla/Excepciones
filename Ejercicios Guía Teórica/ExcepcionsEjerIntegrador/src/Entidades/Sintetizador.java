...
public class Sintetizador implements Usar, Reparar, Arreglar {
    ...
    public Sintetizador(Estado estado, int consumoE) {
        this.estado = estado;
    ...
    public int hablarJARVIS(Armadura arm, int tiempo) throws EnergiaInsuficienteException, DamageDeviceException, DestroyedDeviceException {
        ...
        if (eConsumida > arm.getReactor()) {
            ...
            System.out.println("Energía consumimda al escuchar mensajes por sintetizadopr = " + eConsumida);
            arm.setReactor(arm.getReactor() - eConsumida);
            int probDamage = random.nextInt(10) + 1;
            if (probDamage <= 3) {
                setEstado(Estado.DANIADO);
                System.out.println(" + Alerta!");
                System.out.println("  - Dispositivo Sintetizador dañado");
            }
            return eConsumida;
        }
    }
    ...
    public boolean repararDispositivo(int prob) {
        ...
        int probDamage = random.nextInt(10) + 1;
        if (probDamage <= prob) {
            System.out.println("Dispositivo Sintetizador reparado");
            return true;
        } else {
            System.out.println("El Dispositivo Sintetizador no se pudo reparar");
            return false;
        }
    }
    ...
    public boolean arreglarDispositivo(int prob) throws DestroyedDeviceException {
        ...
        if (probDestruido <= prob) {
            ...
            System.out.println("El Dispositivo Sintetizador no se pudo reparar y quedó destruido");
            return false;
        } else {
